...
class HomesController extends Controller
{
    ...
    public function toTermTf() {
        ...
        foreach($getFilm as $allFilm) {
            ...
            // $nama_film = $allFilm->nama_film;
            // $genre_film = $allFilm->genre_film;
            // $termFilm = $tokenizer->tokenize($nama_film);
    ...
    public function cosine() {
        $this->toTfIdfKuadrat();
        $qTerm = DB::select('SELECT parent.id_film, sum(parent.tf_idf_kuadrat) AS tfidf FROM tf_idfs AS tf_idfs JOIN tf_idfs AS parent ON parent.id_term = tf_idfs.id_term WHERE tf_idfs.id_user = 1 AND parent.id_film >= 1 GROUP BY parent.id_film');

        foreach($qTerm as $qTerms) {
            DB::insert("INSERT INTO cosines(id, id_film, tf_idf_sum) values (null, '$qTerms->id_film', '$qTerms->tfidf')");
        }
        
        $d = DB::select('SELECT SUM(tf_idf) FROM tf_idfs GROUP BY id_film > 1');
        $d = collect($d);
        $d = $d->pluck('SUM(tf_idf)');
        ...
        $q = DB::select('SELECT tf_idf from tf_idfs WHERE id_user >= ?', array(1));
        $q = collect($q);
        $q = $q->pluck('tf_idf');
